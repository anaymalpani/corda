...
/**
 * GracefulReconnect provides the opportunity to perform certain logic when the RPC encounters an error
 * in communication with the node.
 *
 * @param onDisconnect implement this callback to perform logic when the RPC disconnects on an error
 * @param onReconnect implement this callback to perform logic when the RPC has reconnected after an error
 */
data class GracefulReconnect(val onDisconnect: () -> Unit = {}, val onReconnect: () -> Unit = {})
...
class CordaRPCClient private constructor(
    ...
    fun start(username: String, password: String, externalTrace: Trace?, impersonatedActor: Actor?, targetLegalIdentity: CordaX500Name?, gracefulReconnect: GracefulReconnect? = null): CordaRPCConnection {
        ...
        return if (gracefulReconnect != null) {
            CordaRPCConnection.createWithGracefulReconnection(username, password, addresses, gracefulReconnect)
        } else {
